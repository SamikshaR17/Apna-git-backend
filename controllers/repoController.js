...
// getAllRepositories, fetchRepositoryById, fetchRepositoryByName=> ye 3 chize koi bhi search kar sakhta hai, if that repo are public
// All others are only for authorized user

// For getting all repo
async function getAllRepositories(req, res) {
    try {
        // in mongoose, we can directly apply CRUD operations on model itself like, in this case(Repository.find({});)
        // but not in mongodb
...
// current user ki sari repositories
async function fetchRepositoriesForCurrentUser(req, res) {
    ...
    try {
        ...
        console.error("Error during fetching repository of this user: ", err.message);
        res.status(500).send("Server error");
    }
};
...
async function toggleVisibilityById(req, res) {
    ...
    try {
        ...
        console.error("Error during toggling repository's visibility : ", err.message);
        res.status(500).send("Server error");
    }
};
